...
class RavenHandlerTest extends TestCase
{
    ...
    public function testUserContext()
    {
        ...
        $recordWithNoContext = $this->getRecord(Logger::INFO, 'test with default _user context');
        // set _user context 'externally'
        ...
        $recordWithContext = $this->getRecord(Logger::INFO, 'test', array('_user' => $user));

        $ravenClient->user_context(array('id' => 'test_user_id'));
        // handle context
        $handler->handle($recordWithContext);
        $this->assertEquals($user, $ravenClient->lastData['_user']);
